...
export default function LostAndFound() {
  ...
  const fetchData = async (brandId) => {
    setLoading(true);
    setLnfStats([]); // ✅ Clear previous data before fetching new one

    try {
      let requestUrl = `/api/brand/kpis?action=get_lost_found_data`;

      // Use passed brandId or fallback to tokenData
      if (brandId) {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch lost & found KPIs. Status: ${response.status}`
        );
      }
      ...
      setLnfStats(formattedData); // ✅ Even if empty, this will overwrite old data
    } catch (error) {
      console.log("error fetching lost & found data: ", error);
    } finally {
      setLoading(false);
    }
  };
