...
export default function TotalUserRegister() {
  ...
  const fetchData = async (brandId) => {
    ...
    try {
      let response;
      let requestUrl = `/api/brand/kpis?action=total_user_reg_data`;
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch brand KPIs. Status: ${response.status}`
        );
      }
      ...
      setUserStats(formattedData); // ✅ Even if empty, this will overwrite old data
    } catch (error) {
  ...
  return (
    <Container fluid className="p-2" style={{ width: "95%" }}>
      <Row className="align-items-center my-2">
        <Col>
          <h3 style={{ marginBottom: "-4px" }}>Total User Register</h3>
        </Col>
      </Row>
                ...
                >
                  <thead
                    ...
                    style={{
                      ...
                      top: 0,
                      zIndex: 100,
                      backgroundColor: "#f8f9fa", // Light background match
                    }}
                  >
